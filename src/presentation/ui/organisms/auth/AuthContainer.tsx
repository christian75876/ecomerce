...
const AuthContainer = () => {
  ...
  const apiRef = useRef<{
    focusStore: (d?: number) => void;
    focusPlanet: (d?: number) => void;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    if (isRegister) {
      apiRef.current.focusStore(1.2);   
    } else {
      apiRef.current.focusPlanet(1.2);  
    }
  }, [isRegister]);
    ...
    <Box className="relative min-h-screen">
      ...
      <Box
        className="
          ...
          grid place-items-center lg:grid-cols-[1fr_auto] lg:gap-12
          px-6 lg:mr-18
        "
      >
        ...
        <Box
          className={`
            pointer-events-auto w-full max-w-md rounded-2xl p-8
            bg-neutral-gray/20 backdrop-blur-xs shadow-xl
            transition-transform duration-1200 transform
