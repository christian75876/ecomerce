    ...
    <Box className='space-y-8'>
      ...
      <Box className='rounded-[2rem] bg-[linear-gradient(135deg,_#fff7ed_0%,_#ffffff_55%,_#eff6ff_100%)] px-6 py-10 shadow-sm'>
        ...
        <Typography className='mt-3 max-w-2xl text-neutral-dark/70'>
          Administra tus datos personales, notificaciones y configuración de tu tienda.
        </Typography>
      </Box>
      ...
      {loading ? (
        ...
        <>
          ...
          {isAdmin && appConfig ? (
            <div className={`rounded-[1.75rem] border p-6 shadow-sm ${appConfig.isAccessBlocked ? 'border-red-300 bg-red-50' : 'border-neutral-gray/20 bg-white'}`}>
                        ...
                        {store.wppNotificationsEnabled ? (
                          <div className='space-y-2'>
                            ...
                            >
                              ...
                              {storeSaving === store.id + '_save' ? 'Guardando...' : 'Guardar configuración'}
                            </button>
                          </div>
                        ) : null}
                      </div>
                  ...
                  <div className='space-y-4'>
                    {stores.map((store) => (
                      <div key={store.id} className='rounded-2xl border border-slate-200 p-5 space-y-3'>
                        ...
                        >
                          <i className='bx bx-check text-base' aria-hidden='true' />
                          {storeSaving === store.id + '_pay' ? 'Guardando...' : 'Guardar instrucciones'}
                        </button>
                      </div>
                    ))}
                  </div>
